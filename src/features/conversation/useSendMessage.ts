  ...
  const createLastMessage = async (message: string) => {
    if (isGroup) {
      ...
      updateDoc(userChatDocRef, {
        [chatId + ".lastMessage"]: { message, date: Timestamp.now() },
      });
      return;
    }
    ...
    try {
      ...
      const isActive = recipientChatDocData?.[chatId]?.active;
      ...
      updateDoc(recipientChatDocRef, {
        [chatId + ".lastMessage"]: { message, date: Timestamp.now() },
        [chatId + ".unread"]: isActive ? true : recipientChatDocData?.[chatId].unread || true,
        [chatId + ".unreadMsgCount"]: isActive ? increment(1) : recipientChatDocData?.[chatId].unreadMsgCount || 1,
      });
    } catch (error) {
      throw error;
    }
